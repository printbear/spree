...
describe Spree::OrderShipping do
  ...
  describe "#ship" do
    ...
    it "creates a carton with the supplied inventory units" do
      expect { subject }.to change { order.cartons.count }.by(1)
      expect(order.cartons.first.inventory_units).to match_array(inventory_units)
    end

    it "marks the inventory units as shipped" do
      expect { subject }.to change { inventory_units.reload.map(&:state) }.from(['on_hand']).to(['shipped'])
    end
